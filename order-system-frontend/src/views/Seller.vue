  ...
  <div class="container">
    ...
    <div class="table-wrap scrollable">
      ...
      <table class="order-table">
        ...
        <tbody>
          <tr v-for="item in filteredGoodsList" :key="item.id">
            <td>{{ item.id }}</td>
            <td>{{ item.name }}</td>
            <td>{{ item.price }}</td>
            <td>{{ item.category }}</td>
            ...
            <td>
              <button class="btn-sm success" @click="openRestock(item)">补货</button>
              <button class="btn-sm primary" @click="openEdit(item)">编辑</button>
              <button class="btn-sm danger" @click="deleteProduct(item.id)">删除</button>
            </td>
          </tr>
        </tbody>
      </table>
...
export default {
  ...
  methods: {
    ...
    async submitEdit() {
      if(!this.editGoods.name || !this.editGoods.price || this.editGoods.stock === '' || !this.editGoods.category){
        alert('请完善商品信息')
        return
      }
    ...
    async deleteProduct(id) {
      if(confirm('确定要删除该商品？')){
        await axios.post(`http://localhost:8080/order/product/delete/${id}`)
        alert('删除成功')
        this.refreshAll()
      }
    },
...
.table-wrap.scrollable::-webkit-scrollbar-thumb:hover{
  background: #a8a8a8;
}
.order-table{
  width: 100%;
